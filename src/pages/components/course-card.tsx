...
/* ========== MODERN COURSE CARD WITH IMAGE (COMMENTED OUT) ==========
...
========== END MODERN COURSE CARD WITH IMAGE ========== */

// ========== NEW COURSE CARD WITHOUT IMAGE ==========
const CourseCard = ({ id, name, description, price, duration }: CourseCardProps) => {
    return (
        <motion.div
            className="group relative w-full max-w-[380px] h-[420px] bg-gradient-to-br from-white via-gray-50 to-blue-50 rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border-2 border-gray-100 hover:border-primary/30"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, amount: 0.2 }}
            transition={{ duration: 0.5 }}
        ...
        >
            ...
            <div className="p-8 flex flex-col h-full">
                ...
                <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="bg-primary/10 px-4 py-2 rounded-full">
                        <span className="text-xs font-bold text-primary uppercase tracking-wide">Professional</span>
                    </div>

                    {/* Duration Badge */}
                    <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-md">
                        <MdEditCalendar className="text-secondary text-lg" />
                        <span className="text-sm font-bold text-gray-800">{duration} Weeks</span>
                    </div>
                </div>

                {/* Course Title */}
                <h3 className="text-2xl font-black text-gray-900 leading-tight mb-4 line-clamp-2 min-h-[60px]" title={name}>
                    {name}
                </h3>

                {/* Course Description */}
                <p className="text-sm text-gray-600 leading-relaxed line-clamp-4 flex-grow mb-6">
                    {description || "Gain hands-on experience with industry-relevant skills. Expert-led training with practical projects and government-approved certification."}
                </p>
                ...
                <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                        <span className="text-xs text-gray-500 font-semibold uppercase tracking-wide mb-1">Course Fee</span>
                        <span className="text-xl font-black text-primary">
                            ₦{Intl.NumberFormat().format(price)}
                        </span>
                    </div>
                    <Link
                        to={`/course/${id}`}
                        className="bg-gradient-to-r from-blue-500 to-blue-700 text-white px-6 py-3.5 rounded-xl font-bold text-sm shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300 flex items-center gap-2 group/btn"
                    >
                        View Course
                        <FaArrowRight className="group-hover/btn:translate-x-1 transition-transform" />
                    </Link>
            ...
            <div className="absolute -bottom-12 -right-12 w-32 h-32 bg-gradient-to-br from-primary/10 to-secondary/10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500"></div>
        </motion.div>
    )
}
